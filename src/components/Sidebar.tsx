...
const menuItems: { id: PageId; label: string; icon: React.ReactNode }[] = [
  { id: "providers", label: "服务商", icon: <LayoutGrid className="h-4 w-4" /> },
...
function GithubIcon() {
  return (
    <svg className="h-5.5 w-5.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="currentColor"
...
const footerLinks = [
  ...
  {
    label: "GitHub",
    url: "https://github.com/will-17173/freemodel-auto-router",
    icon: <GithubIcon />,
  },
  {
    label: "Bilibili",
    url: "https://space.bilibili.com/328381287",
    icon: <BilibiliIcon />,
  },
]
      ...
      <footer className="mt-auto px-4 pb-4">
        <div className="border-t border-sidebar-border pt-3">
          <div className="mb-2 flex items-center justify-center gap-3">
            {footerLinks.map((link) => (
              <button
                ...
                type="button"
                title={link.label}
                onClick={() => openFooterLink(link.url)}
                className="inline-flex w-12 flex-col items-center gap-1 rounded-md py-1.5 text-[10px] leading-none text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-foreground"
              >
                {link.icon}
          ...
          <div className="text-center text-[11px] leading-4 text-muted-foreground">
            ...
            <span className="ml-1 font-mono">v{version}</span>
          </div>
        </div>
      </footer>
    </div>
